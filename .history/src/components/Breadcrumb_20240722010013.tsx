    ...
    <div className="flex items-center pb-4 whitespace-nowrap">
      {links.map((link:any,ind:any) => (
        ...
        <React.Fragment key={ind}>
          <Link  to={`/${ ind==links.length-1 ? 'courses/'+courseId : courseId} `} className={`${ind==links.length-1?'text-main-color':'text-main-color/75'} hover:underline hover:underline-offset-2`}>
            {link}
          </Link>
