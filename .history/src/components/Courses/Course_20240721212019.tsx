  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden ">
      ...
      <div className="px-6">
        ...
        <p className="opacity-90 font-semibold text-[12px]">
          {course.fullDesc}
        </p>
      </div>
      ...
      <div className="px-6 mt-4 pb-5 flex items-center justify-between">
        <div className="flex gap-2 text-xl font-bold">
          <span className="line-through opacity-75">${course.oldPrice||40}</span>
          <span className="text-[#FD661F] ">${course.price}</span>
        </div>

        <Link to={`/courses/${course.courseId}`}  data-content="Enroll Now" className="relative btn btn-main border-4 bg-main-color text-[14px] font-bold text-white min-w-[max-content!important] px-[12px!important] rounded-[10px!important]">
          Enroll Now
