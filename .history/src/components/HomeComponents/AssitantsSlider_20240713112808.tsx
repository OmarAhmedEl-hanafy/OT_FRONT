...
const assitants = [
  {
    ...
    desc: "Histudy The standard chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested.",
    linksSocial: ["/", "/", "/", "/"],
    classLink: "/",
  },
  {
...
for (let i in imgsAssitants) {
  assitants[i].img = imgsAssitants[i];
}
      ...
      <div className="relative mt-10 py-5 px-[50px] md:px-[70px] mx-3 bg-white border border-[#D0D0D0] rounded-[50px] md:rounded-[100px]">
        ...
        >
          {imgsAssitants.map((img,index) => (
            <div  key={index} className="">
              <div onClick={()=>goTo(index)} className={`h-[100px] w-[100px] md:h-[120px] md:w-[120px] cursor-pointer rounded-full overflow-hidden mx-auto ${ index === activeSlide ? 'border-4 border-main-color box-border' : 'slide'}`}>
                <img className="cursor-pointer"  onClick={()=>goTo(index)} src={img} alt={"Assitant RS"} />
              </div>
            </div>
          ))}
        </Slider>
        ...
        <div className="absolute top-[50%] left-0 right-0 -translate-y-1/2 px-5 flex">
          <button
            ...
            className=" w-[30px] h-[30px] md:w-[40px] md:h-[40px] hover:bg-main-color hover:text-[#FFC224] rounded-full bg-[#FFC224] border"
          >
          ...
          <button
            onClick={previous}
            style={{ boxShadow: "2px 4px 0px #382900" }}
            className="block ml-auto w-[30px] h-[30px] md:w-[40px] md:h-[40px] hover:bg-main-color hover:text-[#FFC224] rounded-full bg-[#FFC224] border"
          >
