function SideBarSingleCourse({course}:any) {
  ...
  course.oldPrice?'':course.oldPrice=course.price+30;

  const discount = ((course.oldPrice-course.price)/course.oldPrice) * 100;

  return (
    <div className="shadow-lg p-3 rounded-lg border border-[#dddddd]">
      <div className="flex justify-between font-semibold items-center">
        <div className="flex gap-4">
          <span className="">${course.price}</span>
          <p className="line-through">${course.oldPrice}</p>
        </div>

        <div className="bg-[#E7F1FF] py-1 px-2 rounded-md">
          <span className="text-main-color">{discount}% OFF</span>
        </div>
      </div>

      <div className="flex gap-2 text-[14px] items-center font-semibold mt-2 border-b-[1px] border-[#dddddd] pb-3">
        <svg
          width={20}
          height={20}
          viewBox="0 0 20 20"
      ...
      <div className="border-b-[1px] border-[#dddddd] pb-3">
        {infoCourse.map((info, ind) => (
          <div
            key={ind}
            className="flex justify-between items-center mt-2 text-[14px] font-semibold "
          >
            <div className="flex gap-2">
              <span className="text-[12px]">{info.icon}</span>
      ...
      <div className="">
        <button
          ...
          className="btn btn-main mt-5 bg-main-color text-white border border-main-color w-full rounded-[5px!important]"
        >
          Add To Cart
        </button>
        ...
        <button
          ...
          className="btn btn-white mt-3 bg-[#E7F1FF] shadow-md font-bold text-main-color w-full rounded-[5px!important]"
        >
         Buy Now
        </button>

        <div className="mt-4 flex gap-5 ">
            <button data-content = "Add to wishlist" className="btn btn-white border border-[#dddddd] rounded-[5px!important] w-full ">Add to wishlist</button>
            <button data-content = "Gift Course"  className="btn btn-white border border-[#dddddd] rounded-[5px!important] w-full">Gift Course</button>
        </div>
