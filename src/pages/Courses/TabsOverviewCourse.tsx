  ...
  return (
    <div className="tabsoverview">
      <div className="w-full">
        <TabGroup>
          <TabList className="flex justify-between border-b-[3px] border-b-[#E9EAF0] md:gap-0">
            {categories.map(({ name }) => (
              <Tab
                key={name}
          ...
          <TabPanels className="mt-3">
            {categories.map(({ name, content }) => (
              <TabPanel key={name} className="rounded-xl bg-bg-main p-3">
                {name === "Instructors" ? (
                  <div className="">
                    {content.map((instructor: any) => (
                      <Instructor key={instructor.id} inst={instructor} />
                    ))}
                  </div>
                ) : (
                  <div className="font-bold">
                    {" "}
                    <h3 style={{ fontSize: "25px" }}>Discription</h3>
                    <br />
                    {course.fullDesc}
                  </div>
                )}
